...
class MorphedEntry extends Relation
{
    ...
    {
        $method = new Method($prop_name = lcfirst(DocInflector::classify($morphItem->isOneToOneRelation() ? DocInflector::singularize($morphItem->getReturnTableName()) : DocInflector::pluralize($morphItem->getReturnTableName()))));
        $method->addImport(Config::workSpace($morphItem->getReturnSchemaName()).'\\'.Helper::className($morphItem->getReturnTableName()));
        $method->setComment('Get all of '.DocInflector::pluralize(Helper::className($morphItem->getReturnTableName())).' that are assigned to this '.Helper::className(DocInflector::singularize($morphItem->getTableName())));
        if ($morphItem->isOneToOneRelation()) {
            $method->setComment('Get  '.DocInflector::singularize(Helper::className($morphItem->getReturnTableName())).' that is assigned to this '.Helper::className(DocInflector::singularize($morphItem->getTableName())));
